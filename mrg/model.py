    ...
    def _get_features(self, user_emb, item_emb, prototype_emb, num_layers=1):
        with tf.variable_scope('features', reuse=tf.AUTO_REUSE):
            ...
            for layer in range(num_layers):
                w = tf.get_variable('w{}'.format(layer), [3 * self.F, 3 * self.F], initializer=self.weight_initializer)
                b = tf.get_variable('b{}'.format(layer), [3 * self.F], initializer=self.const_initializer)
